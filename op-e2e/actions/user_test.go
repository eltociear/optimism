...
func TestCrossLayerUser(t *testing.T) {
	...
	for _, test := range tests {
		testCfg := test // Use a fixed reference as the tests run in parallel
		t.Run(testCfg.name, func(gt *testing.T) {
			runCrossLayerUserTest(gt, testCfg)
		})
	}
}

func runCrossLayerUserTest(gt *testing.T, testCfg regolithScheduledTest) {
	t := NewDefaultTesting(gt)
	dp := e2eutils.MakeDeployParams(t, defaultRollupTestParams)
	dp.DeployConfig.L2GenesisRegolithTimeOffset = testCfg.regolithTime
	...
	require.Equal(t, !testCfg.activateRegolith, infoTx.IsSystemTx())
